...
public interface StorageFacilitiesRepository extends JpaRepository<StorageFacilityType, Integer> {
  ...
  @Query(
      "SELECT SUM(capacity) - SUM(usage) as availStorage "
          + "FROM StorageFacilityType "
  ...
  @Modifying
  @Query(
      "UPDATE StorageFacilityType SET usage = usage + 1 WHERE storageFacilityId = :storageFacilityId")
